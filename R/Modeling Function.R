...
modeling_func = function(df, target_y, title = "", train_span=36, push_span=1, ensemble_n = 300, bagging_prop=0.8, feature_prop=0.8, num_rounds=60, pred_start_date = '2017-01-01', explain_yn = 'N') {
  ...
  for (i in seq(1, by=push_span, to = length(date_unique_bt) - train_span)) {
    ...
    # SHAP
    shap_train <- shap.prep(xgb_model = lgbm_model, X_train = as.matrix(dt_train %>% sample_n(1000) %>% select(-c(`stock_cd`,`date`, contains('target'), contains('return')))))
    shap_test <- shap.prep(xgb_model = lgbm_model, X_train = as.matrix(dt_test %>% select(-c(`stock_cd`,`date`, contains('target'), contains('return')))))
